...
polygon_geojson = dl.GeoJSON(
    url=f"assets/BB_polygons_final.json",
    id="polygon_geojson",
    options={
        "style": {"color": "#3D426B", "opacity": 0.8, "fillOpacity": 0.2, "weight": 3}
    },
...
gemeinde_geojson = dl.GeoJSON(
    url=f"assets/BB_gemeinde_final.json",  # url to geojson file
    ...
    options={
        "style": {"color": "grey", "opacity": 0.5, "fillOpacity": 0.2, "weight": 1}
    },
    hoverStyle=arrow_function(
        {
            "color": "purple",
...
kreis_geojson = dl.GeoJSON(
    url=f"assets/BB_kreis_final.json",  # url to geojson file
    # zoomToBounds=True,  # when true, zooms to bounds when data changes (e.g. on load)
    zoomToBoundsOnClick=True,  # when true, zooms to bounds of feature (e.g. polygon) on click
    options={
        "style": {"color": "grey", "opacity": 0.5, "fillOpacity": 0.2, "weight": 1}
...
tile_layer = dl.TileLayer(
    url="http://193.196.53.65:8070/styles/basic-preview/{z}/{x}/{y}.png",
    id="tile_layer",
...
def get_info(feature=None):
    ...
    if not feature:
        return header + [
            html.P("Zoomen Sie für weitere Statistiken\nauf eine Gemeinde")
        ]
    if feature["properties"]["NAME_3"] is not None:
        if feature["properties"]["suitable_area"] is not None:
            return header + [
                html.B(feature["properties"]["NAME_3"]),
                html.Br(),
                "Potentialfläche in m2: {:.2f}".format(
                    feature["properties"]["suitable_area"]
                ),
                html.Br(),
                "Durchschnittliche Wertung: {:.2f}".format(
                    feature["properties"]["overall_score"]
                ),
                html.Br(),
...
def func(viewport):
    ...
    if (viewport > 8) & (
        state["zoom"] <= 8
    ):  # (((viewport > 7) & (viewport <= 12)) & ((state["zoom"] <= 7) | (state["zoom"] > 12))):
        state["zoom"] = viewport
...
@callback(Output("info", "children"), [Input("gemeinde_geojson", "hover_feature")])
...
@callback(
    [
        Output("card-content-overall", "children"),
...
def info_click(feature):
    ...
    if feature is not None:
        ...
        state["clickedPolygon"] = [
            feature["properties"]["link_id"],
            feature["properties"]["id"],
        ]
        link_name = (
            "Nr. "
            + str(int(feature["properties"]["link_id"][-4:]))
            + "/"
        ...
        try:
            ...
            card_content_image = html.Div(
                [
                    html.Img(src=rgb_image, width=300),
                    html.Br(),
                    html.Span("© GeoBasis-DE/LGB", style={"font-size": "0.8rem"}),
                ]
            )
            #
            graphic_land_cover = html.Img(
                src=svg_handler(
                    "land cover", feature["properties"]["land_cover_score"]
                ),
                id="sym-land",
            ...
            card_content_image = html.Div(
                [
                    html.Span("ⓘ", style={"font-size": "4rem"}),
                    html.Br(),
        ...
        try:
            ...
            graphic_terrain = html.Img(
                src=svg_handler("terrain", feature["properties"]["terrain_score"]),
                id="sym-terrain",
                style={"cursor": "pointer"},
            )
        except:
        ...
        card_content_overall = [
            html.B(link_name),
            html.Br(),
            html.Span("Potentialfläche in m²: ", style={"color": "grey"}),
            suitable_area,
            html.Br(),
            html.Span(
                [
                    html.Span("Gesamtwertung: ", style={"color": "grey"}),
                    overall_score,
                    " ⓘ",
                ],
                id="item-overall-score",
                style={"cursor": "pointer"},
            ),
            html.Br(),
            dbc.Tooltip(
                [
                    "Die Gesamtwertung errechnet sich als ein gewichtetes Mittel der anderen Wertungen. ",
                    html.Br(),
                ...
                target="item-overall-score",
                placement="right",
            ),
            html.Span("Gesamtrang: ", style={"color": "grey"}),
            overall_rank,
            "/",
            max_ranks,
        ]
        ...
        card_footer_overall = [
            html.Img(
                src=svg_handler("overall", feature["properties"]["overall_score"]),
                id="sym-overall",
                style={"cursor": "pointer"},
            ),
            ...
            graphic_terrain,
            dbc.Tooltip(
                ["Wertung der", html.Br(), "Geländebeschaffenheit"],
                target="sym-terrain",
            ),
            #
            html.Img(
                src=svg_handler("grid", feature["properties"]["distance_score"]),
                id="sym-distance",
                style={"cursor": "pointer"},
            ),
            dbc.Tooltip(
                ["Wertung des", html.Br(), "Netzanschlusses"], target="sym-distance"
            ),
            #
            html.Img(
                src=svg_handler(
                    "irradiation", feature["properties"]["irradiation_score"]
                ),
                id="sym-irradiation",
                style={"cursor": "pointer"},
            ),
            dbc.Tooltip(
                ["Wertung des", html.Br(), "Sonnenpotentials"], target="sym-irradiation"
            ),
        ]
        ...
        if state["image_avail"]:
            ...
            land_cover_m2_good = "{:.2f}".format(
                feature["properties"]["land_cover_share_good"]
                * feature["properties"]["suitable_area"]
            )
            # land_cover_share_restricted = "{:.2f}".format(feature["properties"]["land_cover_share_restricted"])
            land_cover_m2_restricted = "{:.2f}".format(
                feature["properties"]["land_cover_share_restricted"]
                * feature["properties"]["suitable_area"]
            )
            ...
            land_cover_score = "{:.2f}".format(
                feature["properties"]["land_cover_score"]
            )
            ...
            df = pd.DataFrame().from_records(
                [
                    {
                        "item": "Niedriger Bewuchs",
                        "value": feature["properties"]["lc_background"],
                    },
                    {"item": "Gebäude", "value": feature["properties"]["lc_building"]},
                    {
                        "item": "Hoher Bewuchs",
                        "value": feature["properties"]["lc_forest"],
                    },
                    {
                        "item": "Landwirtschaft",
                        "value": feature["properties"]["lc_agriculture"],
                    },
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            [
                                html.Span(
                                    "Vollständig nutzbare Fläche in m²: ",
                                    style={"color": "grey"},
                                ),
                                land_cover_m2_good,
                                html.Br(),
                                html.Span(
                                    "Eingeschränkt nutzbare Fläche in m²: ",
                                    style={"color": "grey"},
                                ),
                                land_cover_m2_restricted,
                                html.Br(),
                                html.Span("Wertung: ", style={"color": "grey"}),
                                land_cover_score,
                                html.Br(),
                                html.Span("Gesamtrang: ", style={"color": "grey"}),
                                land_cover_rank,
        ...
        if not state["image_avail"]:
            card_content_land_cover = dbc.Row(
                [
                    html.Span("ⓘ", style={"font-size": "1.5rem"}),
                    html.Span(
                        "Mangels Bildinformation kann die Landbedeckung nicht beurteilt werden"
                    ),
                ],
        ...
        if state["image_avail_terrain"]:
            ...
            card_content_terrain = dbc.Row(
                [
                    dbc.Col(
                        [
                            html.Img(src=terrain_image, width=300),
                            html.Br(),
                            html.Span(
                                "© GeoBasis-DE/LGB", style={"font-size": "0.8rem"}
                            ),
                        ],
                        xs=12, md=6,
                        class_name="h-100 justify-content-center align-items-center",
                    ),
                    dbc.Col(
                        [
                            html.Span(
                                "Mittlere Abweichung der Steigung in Grad: ",
                                style={"color": "grey"},
                            ),
                            terrain_roughness,
                            html.Br(),
                            html.Span("Hochpunkt: ", style={"color": "grey"}),
                            terrain_high,
                            html.Span(
                                "  Tiefpunkt: ",
                            ...
                            html.Span(
                                ...
                                id="item-terrain-score",
                                style={"cursor": "pointer"},
                            ),
                            html.Br(),
                            dbc.Tooltip(
                                [
                                    "Die Geländebeschaffenheit wird relativ zum unteren Referenzwert ",
                                    "von 90 Grad bewertet",
                                ],
                                delay={"show": 20, "hide": 50},
                                target="item-terrain-score",
                            ),
        ...
        if not state["image_avail"]:
            card_content_terrain = dbc.Row(
                [
                    html.Span("ⓘ", style={"font-size": "1.5rem"}),
                    html.Span(
                        "Mangels Terraininformation kann die Geländebeschaffenheit nicht beurteilt werden"
                    ),
                ],
                class_name="text-center",
            )
        ...
        # get distance data
        grid_subset = grid.loc[
            (
                (grid["link_id"] == state["clickedPolygon"][0])
                & (grid["id"] == state["clickedPolygon"][1])
            ),
            :,
        ].sort_values("distance_substation")
        distance = "{} in {:.0f}m".format(
            grid_subset["municipality"][grid_subset.distance_substation.idxmin()],
            grid_subset["distance_substation"][
                grid_subset.distance_substation.idxmin()
            ],
        )
        distance_score = "{:.2f}".format(feature["properties"]["distance_score"])
        distance_rank = "{:.0f}".format(feature["properties"]["distance_rank"])
        #
        card_content_grid = [
            html.Span("Nächster Netzanschlusspunkt: ", style={"color": "grey"}),
            html.Br(),
            "  ",
            distance,
            html.Br(),
            html.Span(
                [html.Span("Wertung: ", style={"color": "grey"}), distance_score, " ⓘ"],
                id="item-grid-score",
                style={"cursor": "pointer"},
            ),
            html.Br(),
            dbc.Tooltip(
                [
                    "Der Netzanschluss wird relativ zum schlechtesten ",
                    "in Brandenburg beobachteten Wert bewertet",
                ],
                delay={"show": 20, "hide": 50},
                target="item-grid-score",
            ),
            html.Span("Rang: ", style={"color": "grey"}),
            distance_rank,
            "/",
            ...
            html.Br(),
            dbc.Button(
                "Nächstgelegene 3 Netzanschlusspunkte anzeigen",
                id="show-grid-access",
                color="primary",
                class_name="mr-1 float-end",
        ...
        irradiation_score = "{:.2f}".format(feature["properties"]["irradiation_score"])
        irradiation_rank = "{:.0f}".format(feature["properties"]["irradiation_rank"])
        #
        card_content_irradiation = [
            html.Span("Durchschnittliche Wh/m² pro Monat: ", style={"color": "grey"}),
            irradiation,
            html.Br(),
            html.Span(
                [
                ...
                id="item-irradiation-score",
                style={"cursor": "pointer"},
            ),
            html.Br(),
            dbc.Tooltip(
                [
                    "Das Sonnenpotential wird relativ zum schlechtesten ",
                    "in Deutschland beobachteten Wert bewertet",
                ],
                delay={"show": 20, "hide": 50},
                target="item-irradiation-score",
            ),
            html.Span("Rang: ", style={"color": "grey"}),
            irradiation_rank,
            "/",
        ...
        card_content_sources = [
            dcc.Link(
                "OpenStreetMap", href="https://www.openstreetmap.org/", target="_blank"
            ),
            ", ",
            dcc.Link(
                "DOP20: GeoBasis-DE/LGB",
                href="https://geobroker.geobasis-bb.de/gbss.php?MODE=GetProductPreview&PRODUCTID=7a503f0f-db46-4772-80e3-b27733fd7acd",
                target="_blank",
            ),
            ", ",
            dcc.Link(
                "DGM01: GeoBasis-DE/LGB",
                href="https://geobroker.geobasis-bb.de/gbss.php?MODE=GetProductPreview&PRODUCTID=414f568f-639b-4b5a-ba92-57fdac396799",
                target="_blank",
            ),
            ", ",
            ...
            dcc.Link(
                "CM SAF",
                href="https://doi.org/10.5676/EUM_SAF_CM/SARAH/V002",
                target="_blank",
            ),
            html.Br(),
        ]
...
@callback(
    [Output("grid-access", "children"), Output("show-grid-access", "children")],
    Input("show-grid-access", "n_clicks"),
)  #
def info_click(n):
    if n is None:
        raise PreventUpdate()
        ...
        return [
            dl.Marker(position=(tmp["lat_substation"][i], tmp["lon_substation"][i]))
            for i in tmp.index
        ], "Nächstgelegene 3 Netzanschlusspunkte ausblenden"
    else:
                    ...
                    [
                        dl.Map(
                            children=[
                                ...
                                dl.Pane(
                                    [kreis_geojson],
                                    name="regional-polygon",
                                    id="regional-polygon",
                                    style={"z-index": 200},
                                ),
                                dl.Pane(
                                    [polygon_geojson],
                                    name="highway-polygon",
                                    style={"z-index": 300},
                                ),
                                dl.Pane(
                                    dl.FeatureGroup(id="grid-access"),
                                    name="grid-access-pane",
                                    style={"z-index": 350},
                                ),
                                ...
                                dbc.Card(
                                    [
                                        dbc.CardBody(
                                            [
                                                html.H4(
                                                    "Quellen", className="card-title"
                                                ),
                                                html.Div(
                                                    [],
                                                    id="card-content-sources",
